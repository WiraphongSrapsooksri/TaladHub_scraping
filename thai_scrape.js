...
async function safeGoto(page, url, retries = 3) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.error(`Failed to load page, retrying... (${i + 1}/${retries})`);
      if (i === retries - 1) {
        throw error;
      }
    }
...
async function fetchMarketList(browser, page) {
  ...
  // // const url = `https://thaimarket.biz/markets?page=5`;
  // const pageInstance = await browser.newPage();
  // await pageInstance.goto(url, { waitUntil: "networkidle0" });
  const url = `https://thaimarket.biz/markets?page=${page}`;
  const pageInstance = await browser.newPage();
  await safeGoto(pageInstance, url);
...
async function fetchMarketDetails(browser, market) {
  ...
  // await pageInstance.goto(market.linkDetail, { waitUntil: "networkidle0", timeout: 60000 });
  ...
  try {
    ...
    //   timeout: 5000,
    // });
    await safeGoto(pageInstance, market.linkDetail);

    await pageInstance.waitForSelector("h1.text-3xl.font-semibold", {
      timeout: 60000,
    });

    // await pageInstance.waitForSelector("h1.text-3xl.font-semibold", { timeout: 60000 });
    ...
    // );
    const ratingDetail = await pageInstance.$eval(
      'span.font-semibold.text-sm.text-secondary-500',
      (el) => el.textContent.trim()
    );
...
async function fetchMarkets(startPage,maxPages) {
  ...
  try {
    ...
    const dataFilePath = path.join(__dirname, "data", "DataThaiMarket.json");
...
// const maxPages = 15;
// const startPage = 11
// fetchMarkets(startPage,maxPages);

const startPage = parseInt(process.argv[2], 10); 
const endPage = parseInt(process.argv[3], 10); 
fetchMarkets(startPage, endPage);
